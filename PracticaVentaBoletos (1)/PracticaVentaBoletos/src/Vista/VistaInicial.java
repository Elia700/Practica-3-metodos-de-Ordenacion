...
public class VistaInicial extends javax.swing.JFrame {
    ...
    DynamicList<Pasajero> ListaPasajero = new DynamicList<>();
    ...
    private void Guardar() throws EmptyList, ParseException {
        ...
        if (txtNumeroDNI.getText().isEmpty()) {
            ...
            Integer IdPasajero = ListaPasajero.getLength() + 1;
    ...
    private void ordenar() {
        ...
        if (btnOrden.isSelected()) {
            tipo = 1;
        } else if (btnOrden1.isSelected()) {
            tipo = -1;
        }

        try {
            DynamicList<Pasajero> listaOrdenada;
            long tiempoInicio = System.currentTimeMillis(); 

            String metodoSeleccionado = cbxMetodos.getSelectedItem().toString();
            switch (metodoSeleccionado) {
                case "quickSort":
                    listaOrdenada = controlPasajeros.quicksort(controlPasajeros.all(), tipo, criterio);
                    break;
                case "shellSort":
                    listaOrdenada = controlPasajeros.shellsort(controlPasajeros.all(), tipo, criterio);
                    break;
                default:
                    throw new IllegalArgumentException("Método de ordenamiento no válido: " + metodoSeleccionado);
            }

            long tiempoFin = System.currentTimeMillis(); 
            long tiempoEjecucion = tiempoFin - tiempoInicio; 

            modelotabla.setListaPasajero(listaOrdenada);
            tblVentas.setModel(modelotabla);
            tblVentas.updateUI();

            JOptionPane.showMessageDialog(null, "Tiempo de ejecución: " + tiempoEjecucion + " ms", "INFO", JOptionPane.INFORMATION_MESSAGE);
        } catch (Exception e) {
    ...
    private void CargarVista() {
        ...
        if (fila < 0) {
            ...
            try {
                ...
                cbxHoraSalida.setSelectedItem(controlPasajeros.getPasajero().getDatosBoleto().getHoraViaje().toString());
    ...
    private void initComponents() {
        ...
        cbxCriterio.setModel(new javax.swing.DefaultComboBoxModel<>(new String[] { "Nombre", "Apellidos", "NumeroIdentificacion" }));
        ...
        cbxMetodos.setModel(new javax.swing.DefaultComboBoxModel<>(new String[] { "quickSort", "shellSort" }));
    ...
    private void btnComprarBoletoActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnComprarBoletoActionPerformed
        if (txtNumeroDNI.getText().isEmpty()) {
            ...
            try {
                ...
                Integer IdPersona = ListaPasajero.getLength() + 1;
                String Origen = cbxOrigen.getSelectedItem().toString();
                String Destino = cbxDestino.getSelectedItem().toString();
                ...
                ListaPasajero.add(PasajeroGuardar);
